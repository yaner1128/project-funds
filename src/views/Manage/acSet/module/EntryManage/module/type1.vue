<template>
  ...
  >
    ...
    <voucherVue ref="voucherRef"></voucherVue>
  </el-dialog>
</template>
  ...
  setup(props, { emit }) {
    const data = reactive({
      ...
      handleSelectionChange: (selection: any) => {
        if(!data.selectRowsList.includes(selection.code)) {
          data.selectRowsList.push(selection.code);
          return;
        }
        data.selectRowsList.splice(data.selectRowsList.indexOf(selection.code), 1)
      },
